...
# mirror the real browser POST exactly
body = json.dumps({"provider": "basic", "username": "keovoin", "password": PW}).encode()
req = urllib.request.Request(BASE + "/auth/password-login", data=body, method="POST")
req.add_header("Content-Type", "application/json")
req.add_header("Referer", BASE + "/login?next=/")
try:
    r = op.open(req, timeout=40)
    print("login ->", r.status)
except urllib.error.HTTPError as e:
    print("login ->", e.code, e.read(300))
